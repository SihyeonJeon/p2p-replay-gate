...
```bash
p2p-replay-gate import-xes --pack bpic2019 --input BPI_Challenge_2019.xes.gz --output imported/bpic2019.jsonl --report imported/bpic2019_adapter.json --strict
...
For the full BPIC2019 log, run once without `--strict`, review `unmapped_activities`, then rerun with `--strict` after the audit scope is fixed.
